...
| `buildctl_fallback` | append-only entries with timestamp, phase, reason (`runtime-not-found`, `node-version`, or `execution-unavailable`), and detail | root, only when buildctl cannot execute | never (audit trail); runnable diagnostics never append fallback |
...
14. **Compiled artifacts never own transitions.** Root is the only state writer. A runnable `validate-plan` failure blocks Plan acceptance, and stale/invalid receipt checks never select prompt fallback. buildctl generates contracts/receipts/ledgers only; it has no workflow mutation or `complete-slice` authority.
