...
10. **Validate the execution_manifest.** Every manifest task must include `id`, `wave`, `depends_on`, `files_modified`, `requirements`, `must_haves`, `verify`, and `done`. Validate the typed evidence contract: new plans use `evidence_mode: typed`; each must-have has exactly `id`, `claim`, and `evidence`; evidence has exactly `kind` and `ref`; supported kinds are `behavioral-test`, `command-assertion`, `structural`, and `manual-receipt`. A missing field, duplicate ID, empty ref, or unsupported evidence kind is **Important**; missing `files_modified` or `requirements` is **Critical**.
    Cross-check every named symbol, behavior, and invariant in Approach against `bindings`, whose entries have exactly `id`, `kind`, `name`, `task_id`, and `must_have_id`. Each reference must exist and the must-have must belong to the named task. An unbound Approach obligation is **Important**. Structural evidence cannot prove behavior.
    Check evidence atomicity, not task size alone. Every must-have must be independently observable in one bounded implement-verify cycle; when unrelated APIs, migrations, fixtures, or invariants share only one broad result, require a split. A non-atomic task is **Important**. Reopened `legacy-untyped` tasks must upgrade to typed must-haves and bindings; unchanged completed legacy tasks remain valid, but their behavioral strings still need direct evidence.
