# Kemet-lite unrelated behavioral evidence
...
execution_manifest:
  - id: T-001
    ...
    files_modified: [src/legacy-pose.js, test/unrelated.test.js]
    requirements: [REQ-001]
    decisions: [D-001]
    must_haves:
    ...
    done: "an unrelated green test is present"
```
...
delivery_slices:
  - id: S-001
    goal: "deliver legacy building-pose adoption"
    depends_on: []
    task_ids: [T-001]
    requirements: [REQ-001]
    must_haves: ["the legacy building pose is adopted"]
    verify: ["node --test test/unrelated.test.js"]
    done: "REQ-001 is claimed complete"
```
