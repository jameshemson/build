...
- **One state file**: read it. If $ARGUMENTS is empty or describes the same work as its `task:` field, resume at the recorded phase. When resuming, compare `git branch --show-current` with the state's `branch:` field; if they differ, check out the workflow branch before doing anything else. When `agent_progress` holds a relay attempt with a null `terminal_status`, apply the resume identity check in [implement relay](reference/implement-relay.md) before that checkout. If state has no `branch:` field (pre-upgrade workflow), note that and continue on the current branch. If $ARGUMENTS describes different work, leave that workflow untouched and start a fresh one with a new slug.
...
3. Invoke `/build:impl-plan` via the Skill tool for: [orchestrated] $ARGUMENTS. When the mode routes `plan` to the fable model (`fable` and `mixed`): in a fable session, execute the `impl-plan` protocol in the root session — read `../impl-plan/SKILL.md` and its references and author the plan directly — recording `plan: active-session`; in any other session, dispatch a fresh agent with `model: fable` carrying the full planning context, recording `plan: fable`; on an unavailable or rejected override, append `model_fallback`, execute in the root session, and disclose it.
...
8. Resolve the sibling `buildctl/cli.js` relative to this skill (or package `buildctl` bin; source checkouts use `source/skills/build/buildctl/cli.js`) with Node >=20. Run `validate-plan --plan .build/plans/{slug}-plan.md --out .build/contracts/{slug}/contract.json`. Record its path, plan/contract hashes, and compiler version as `compiled_contract`; a runnable diagnostic blocks and keeps `phase: plan`. Runnable `check-counters` and `complete-slice` diagnostics are equally authoritative. Only runtime-not-found/execution-unavailable selects prompt validation and appends `buildctl_fallback`. Markdown/YAML remains authored authority and generated JSON is never edited.
...
3. **Deploy agents inside the active slice**: In `mixed` mode with a Build-default `implement` route, the active slice is one implement relay: follow [implement relay](reference/implement-relay.md) instead of dispatching workstream agents, and on an accepted `done` outcome continue at step 7; a named `implement` agent route still dispatches that agent. Preserve the hierarchy delivery slice → dependency waves → disjoint workstreams → `execution_manifest` tasks. Prefer the manifest and route only task IDs in `active_slice.task_ids`, by `wave`, `depends_on`, and `files_modified`. Never dispatch a later slice. If the manifest is absent or malformed, report that and use prose order/workstreams only when their active-slice ownership is unambiguous; otherwise return to planning:
...
7. **Mid-review gate**: After all workstreams for the active slice complete and merge (before its checkpoint), spawn a **Sonnet agent** for mid-review (Phase 3b). Pass it the plan, review, state, requirements, context, and implementation-summary paths plus a summary of what was built. For complex changes (state says `complexity: complex`), also run mid-reviews after each major workstream completes. When the agent returns, address any fixes needed. For a relayed slice, address them through the repair handoff in [implement relay](reference/implement-relay.md). If it returns RETHINK, treat as a scope change — return to Phase 1 with rework notes.
8. **Checkpoint the active slice in strict order**: after integration, run the slice's exact `verify` and observable `must_haves`; update the implementation summary with that provisional evidence; and make the root checkpoint commit. For a relayed slice the relay section is that provisional evidence; root still runs the integration command, commits, and runs post-checkpoint `run-evidence` itself, and never implements a relayed slice inline. Record its commit ID in the summary as the full SHA in `checkpoint_commits` plus exact `Completion checkpoint: {"slice_id":"S-###","commit":"<40-lowercase-hex>"}` marker. Run post-checkpoint `run-evidence` for every slice command and author current structural/manual/slice judgment YAML without auto-stamping acceptance. Run `complete-slice`; blocked leaves the slice active. For `proposed`, root validates the receipt, applies only `append_completed_slice`, `set_active_slice`, `append_transition_reference`, and `append_history_template`, then reruns and requires `already_applied`; the authorized patch already performed slice completion and next-slice activation, so never append to `completed_slices` or set `active_slice` again for that receipt. Any evidence, integration, summary-write, or commit failure leaves the same `active_slice` in place and blocks every later slice. Recorded genuine buildctl unavailability uses and discloses the prompt-owned checkpoint path; runnable diagnostics never select fallback. Repeat Phase 3 from the patch-applied `active_slice`.
9. Only when every slice is complete and `active_slice: null`, validate the final implementation summary. Root runs `buildctl run-evidence` for every compiled exact command plus the repository-required final full-suite command and records `evidence_ledger`; a valid failed-command ledger proceeds to Verify judgment, but runner/stability errors stay in implementation. In recorded prompt fallback, Verify executes the ledger itself. buildctl never writes workflow state or mutates git; root alone applies an allowed completion patch and owns general phase transitions. Then update state to `phase: verify`, append history, and auto-continue to Phase 3c. Slice evidence is provisional and never substitutes for fresh whole-workflow verification.
...
3. Invoke `/build:architect-review` via the Skill tool with explicit context: workflow slug, state path, `{slug}-verify.md` path, verification verdict, and review target `git diff {base_ref}...HEAD` (or `git diff HEAD` if `base_ref` is unavailable). In `mixed` mode this phase is a relay stop: follow the relay contract in [workflow modes](reference/workflow-modes.md) instead of invoking the skill, and on resume validate the relayed artifact before `compile-result`. When the mode routes `architect-review` to the fable model (`fable`): in a fable session, execute the `architect-review` protocol in the root session with the same explicit context, recording `architect-review: active-session`; in any other session, dispatch a fresh agent with `model: fable` carrying that context, recording `architect-review: fable`; on an unavailable or rejected override, append `model_fallback`, execute in the root session, and disclose it.
...
Before every dispatch or transition, root appends the typed occurrence to `counter_events` and runs `buildctl check-counters --state .build/plans/{slug}-state.md`; root owns the occurrence and buildctl owns deterministic counting. An authoritative `halt` writes the matching halt fields. Only recorded genuine runtime unavailability uses the prompt counts below.
...
- **Phase agent failure**: If a spawned phase agent (e.g. mid-review) errors or returns output missing its required verdict, re-dispatch once with the expected output format restated verbatim. On a second failure, halt with `phase-agent-failure`, log both failures under `agent_failures:`, and escalate to the user — `check-counters` returns an authoritative halt at that same second failure, so never replace an independent phase agent with inline execution. A phase is never skipped because its agent failed.
...
- **Workstream agents never touch `.build/`.** Artifacts are orchestrator-owned and invisible inside worktrees; the only exception is a fallback implement coordinator executing the `implement-slice` protocol, whose natural artifacts are its slice section, the progress log, and its named evidence directory.
