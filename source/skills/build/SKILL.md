...
8. **Checkpoint the active slice in strict order**: after integration, run the slice's exact `verify` and observable `must_haves`; update the implementation summary with that provisional evidence; and make the root checkpoint commit. Record its commit ID in the summary as the full SHA in `checkpoint_commits` plus exact `Completion checkpoint: {"slice_id":"S-###","commit":"<40-lowercase-hex>"}` marker. Run post-checkpoint `run-evidence` for every slice command and author current structural/manual/slice judgment YAML without auto-stamping acceptance. Run `complete-slice`; blocked leaves the slice active. For `proposed`, root validates the receipt, applies only `append_completed_slice`, `set_active_slice`, `append_transition_reference`, and `append_history_template`, then reruns and requires `already_applied`. Only then append the slice to `completed_slices` and activate the next declared-order dependency-ready incomplete slice. Any evidence, integration, summary-write, or commit failure leaves the same `active_slice` in place and blocks every later slice. Recorded genuine buildctl unavailability uses and discloses the prompt-owned checkpoint path; runnable diagnostics never select fallback. Repeat Phase 3 for the next slice.
