...
The state file (`.build/plans/{slug}-state.md`) is the source of truth for the workflow. The orchestrator is its only writer. Workstream agents in isolated worktrees must never write to it.
...
| `complexity` | `simple` or `complex` | Phase 1 | never |
...
| `completed_tasks` | `T-###` IDs only — no checksums or commit IDs | Phase 3, after wave merge + integrated verification | cleared per-task when a file revert or rework names the task |
...
| `merge_conflicts` | files + workstreams that conflicted | Phase 3 merge protocol | never (audit trail) |
